...
![Slide from Jenny Bryan (Code Smells and Feels)](images/hedgehog-jenny-bryan.png){fig-align="center" width="80%"}
...
One approach is to treat each branch as very temporary: you make a branch from main, add your contribution, make a pull request, merge and delete the branch. This is the approach with the NASA Mentors and it's more continuous little updates rather than larger big updates, so we can make progress together more quickly. This also works better when working with Jupyter Notebooks, which can cause merge conflicts.
